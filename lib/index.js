...
const buildFile = (item, key, gcKey, opts) => {
  ...
  for (const [key, gcKey] of Object.entries(opts.mappings)) {
    ...
    if (val && gcKey === 'Content_Content') {
      ...
      (
        ...
        (lowerKey.lastIndexOf('video__file') > -1 && lowerKey.lastIndexOf('video__file') === lowerKey.length - 'video__file'.length)
      )
    ) {
...
const saveFiles = (opts, done) => {
  ...
  Object.entries(filesToSave).forEach(file => {
    ...
    request(origin).pipe(fs.createWriteStream(destFileName)).on('close', () => {
      ...
      fs.rename(destFileName, `${destFileName}.${type.ext}`, function (err) {
        if (err) {
...
const postParsing = (filesToSave, flatData, metalsmith, done, opts) => {
...
module.exports = function (opts) {
  return (files, metalsmith, done) => {
    ...
    if (opts.useLocalData && fs.existsSync('parsed.json')) {
      ...
      flatData = JSON.parse(fs.readFileSync('parsed.json', { encoding: 'utf8' }))
