...
module.exports = function (opts) {
  return function (files, metalsmith, done) {
    ...
    .then(results => {
      ...
      results.forEach(el => {
        //process.exit()
        let thisHasFiles = hasFiles(el)
        console.log(`${el.data.name} - HasFiles: ${thisHasFiles}`)

        if (thisHasFiles) {
          getFiles(el.data.id).then(jsonOutput => {
...
function hasFiles(el) {
  for (let element of el.data.config[0].elements) {
        ...
        if (el.config[0]) {
          if (el.config[0].elements) {
            ...
            el.config[0].elements.forEach((el, index) => {
              ...
              let component = newEl.components[index]
              component = {
                type: el.type,
                name: el.name
              }
              ...
              if (el.subtitle) {
                component.subtitle = el.subtitle
              }
              if (el.label) {
